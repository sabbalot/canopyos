    ...
    async def run_update(self, update_id: str) -> None:
        ...
        try:
            ...
            if sess.cancel_requested:
                return
            await self.emit(sess, "backup", "Creating backups", 20)
            ...
            # If targets known, pin forward using temporary override
            if targets:
                override_path = os.path.join("/tmp", "compose.override.forward.yml")
                content_lines = ["services:"]
                if "app" in targets:
    ...
    async def _attempt_rollback(self, prev_map: Dict[str, str], sess: UpdateSession) -> None:
        try:
            ...
            if not app_repo or not app_digest or not be_repo or not be_digest:
                ...
                return
            workdir = os.environ.get("WORKDIR", "/workspace")
            override_path = os.path.join("/tmp", "compose.override.rollback.yml")
            content = (
                "services:\n"
            ...
            ok = await self._compose(["-f", "docker-compose.yml", "-f", override_path, "up", "-d", "--force-recreate"], sess)
            if not ok:
