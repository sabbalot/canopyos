...
class UpdaterOrchestrator:
    ...
    async def _compose(self, args: List[str], sess: Optional[UpdateSession] = None) -> bool:
        """
        Execute docker compose directly using the mounted docker.sock and project directory.
        Avoids Docker-in-Docker and path translation issues.
        """
        workdir = os.environ.get("WORKDIR") or "/opt/canopyos"
        project_name = os.environ.get("COMPOSE_PROJECT_NAME", "canopyos")

        # Detect migration run to avoid explicit -p due to secret prefix issues
        is_migration = "run" in args and "migrations" in args

        # Include pinned override automatically when present, unless caller already provided -f
        pinned_path = os.path.join(workdir, "docker-compose.pinned.yml")
        pinned_exists = os.path.exists(pinned_path)
        ...
        if pinned_exists and "-f" not in args:
            cmd += ["-f", "docker-compose.yml", "-f", "docker-compose.pinned.yml"]
        cmd += args

        # Log command being executed
        if sess:
            await self._write_log(sess, f"Executing: {' '.join(cmd)} (cwd={workdir})")
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    pass
                if sess:
                    await self.emit(sess, "failed", "Timeout executing compose command", sess.progress)
                return False
            finally:
                await proc.wait()

            if proc.returncode != 0:
                logger.error(f"Compose command failed with exit code: {proc.returncode}")
                if sess:
                    sess.log_lines.append(f"ERROR: Compose failed with exit code {proc.returncode}")
                    await self._write_log(sess, f"ERROR: Compose failed with exit code {proc.returncode}")
